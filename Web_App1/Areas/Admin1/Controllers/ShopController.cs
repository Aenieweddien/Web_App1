...
using Web_App1.Models.ViewModels.Shop;
using System;
...
namespace Web_App1.Areas.Admin.Controllers
{
    public class ShopController : Controller
    {
      //  [Authorize(Roles = "Admin,Manager")]
        ...
        [HttpPost]
        public ActionResult AddProduct(comicsVM model,HttpPostedFileBase file) {
            ...
            using (DB db = new DB())
            {
                comics product = new comics();
                product.Title = model.Title;
                product.Slug = model.Title.Replace(" ", "-").ToLower();
            ...
            using (DB db = new DB())
            {
                comics dto = db.Products.Find(id);
                dto.Image = ImageName;
                db.SaveChanges();
            }
        ...
        [HttpPost]
        public ActionResult EditProduct(comicsVM model, HttpPostedFileBase file) {
            int id = model.Id_comic;
            ...
            using (DB db = new DB()) {
                if (db.Products.Where(x => x.Id_comic != id).Any(x => x.Title == model.Title)) {
        ...
        public ActionResult Orders() {
            ...
            using (DB db = new DB())
            {
                ...
                foreach (var order in orders) {
                    ...
                    {
                        comics product = db.Products.FirstOrDefault(x => x.Id_comic == orderDetails.ProductId);
                        decimal price = product.New_Price;
